...
pub static TOOL_REGISTRY: &[ToolInfo] = &[
    ...
    ToolInfo {
        ...
        full_desc: "Resolve a module from debug_modules and open its on-disk image in a newly allocated workspace database. idb_out is always required because runtime modules commonly live in read-only system directories. The module must have an on-disk image: macOS system libraries are dyld-shared-cache-resident with no file on disk, so open those with open_dsc/dsc_add_dylib instead. Returns the new database_id and a checked runtime slide; it does not replace the active debug database.",
        example: r#"{"database_id":"source UUID","module":"/opt/homebrew/lib/libplugin.dylib","idb_out":"/tmp/libplugin.i64"}"#,
        default: false,
